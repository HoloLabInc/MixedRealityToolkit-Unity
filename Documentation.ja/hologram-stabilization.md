...
安定なホログラフィック レンダリングは、プラットフォームとデバイスによるヘッド ポーズ トラッキングに大きく依存します。Unity は、基礎となるプラットフォームによって推定および提供されるカメラ ポーズからのフレームごとにシーンをレンダリングします。このトラッキングが実際のヘッドの動きに正確に追従しない場合、ホログラムは視覚的に不正確に見えます。HoloLens のような AR デバイスでは、ユーザーが仮想ホログラムを現実世界に関連付けることができるため、これは特に明白で重要です。パフォーマンスは、信頼性の高いヘッド トラッキングには重要ですが、[HoloLens の環境に関する考慮事項](https://docs.microsoft.com/ja-jp/hololens/hololens-environment-considerations) も重要です。ユーザ エクスペリエンスに影響を与える環境要素のタイプは、対象となるプラットフォームの仕様によって異なります。
